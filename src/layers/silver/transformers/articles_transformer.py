...
def transform_articles(full_refresh: bool = True):
    ...
    print(f"[{start_time.strftime('%H:%M:%S')}] Iniciando transformación de artículos...")
    ...
    with engine.connect() as conn:
        ...
        print(f"[{delete_start.strftime('%H:%M:%S')}] Full refresh: eliminando datos de silver.articles...")
        ...
        if total == 0:
            print("Sin datos para procesar en bronze.raw_articles")
            cursor.close()
            return
        ...
        print(f"\n{'='*60}")
        print(f"Transformación completada: {inserted:,} artículos insertados")
