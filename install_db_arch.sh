...
# ==========================================
# 1. INSTALACIÓN DE SISTEMA (Arch Linux)
# ==========================================
echo "=== 1. Instalando PostgreSQL y dbmate ==="
...
# Instalar dbmate si no esta disponible
if ! command -v dbmate &> /dev/null; then
    echo "=== Instalando dbmate ==="
    # Intentar desde repositorios oficiales, si no, desde AUR
    if pacman -Si dbmate &> /dev/null; then
        sudo pacman -S --needed --noconfirm dbmate
    else
        echo "dbmate no esta en los repositorios oficiales."
        echo "Instalar manualmente desde AUR: yay -S dbmate-bin"
        echo "O descargar el binario desde: https://github.com/amacneil/dbmate/releases"
        exit 1
    fi
fi
...
PGPASSWORD="$DB_PASSWORD" DATABASE_URL="postgres://$DB_USER:$DB_PASSWORD@localhost:5432/$DB_NAME?sslmode=disable" \
    dbmate --migrations-dir "./sql/migrations" --no-dump-schema up
